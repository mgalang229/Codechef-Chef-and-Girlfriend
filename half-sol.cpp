...
int main() {
	...
	while (tt--) {
		...
		// for the first plan: find the total minutes between 'time1' and 'time2' plus the time that the two of them goes back home
		...
		// note: don't forget to remove the 'dist' from 'first' to remove the time that the two of them goes back home
		first -= dist;
		if (dist * 2 <= first) {
			cout << first;
		} else {
